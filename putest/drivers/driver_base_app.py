...
class DriverBaseApp(object):
    ...
    def init():
        try:
            ...
            if Var.driver.lower() == 'appium':
                from putest.drivers.appium import AndroidDriver, iOSDriver
            else:
                from putest.drivers.macaca import AndroidDriver, iOSDriver
    ...
    def find_elements_by_key(key, timeout=10, interval=1, index=0, not_processing=False):
        ...
        if Var.desired_caps.platformName.lower() == 'android':
            ...
            elif re.match(r'android\.[a-zA-Z]+[\.(a-zA-Z)]+', key) or re.match(
                    r'[a-zA-Z]+\.[a-zA-Z]+[\.(a-zA-Z)]+', key):
                dict['element_type'] = 'classname'
            elif re.match('//\*\[@\S+=\S+\]', key) or re.match(
                    '//[a-zA-Z]+\.[a-zA-Z]+[\.(a-zA-Z)]+\[\d+\]', key):
    ...
    def wait_for_elements_by_key(elements_info):
        ...
        log_info(" --> body: {'using': '%s', 'value': '%s', 'index': %s, 'timeout': %s}" % (
        element_type, element, index, timeout))
        if element_type == 'name':
            elements = driver.wait_for_elements_by_name(name=element, timeout=timeout,
                                                        interval=interval)
        elif element_type == 'id':
            elements = driver.wait_for_elements_by_id(id=element, timeout=timeout,
                                                      interval=interval)
        elif element_type == 'xpath':
            elements = driver.wait_for_elements_by_xpath(xpath=element, timeout=timeout,
                                                         interval=interval)
        elif element_type == 'classname':
            elements = driver.wait_for_elements_by_classname(classname=element, timeout=timeout,
                                                             interval=interval)
        else:
            elements = None
        ...
        if elements:
            ...
            if len(elements) <= int(index):
                log_error('elements exists, but cannot find index({}) position'.format(index),
                          False)
