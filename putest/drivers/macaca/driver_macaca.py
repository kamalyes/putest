...
class AndroidDriver(object):
    ...
    def adb_shell(cmd):
        ...
        try:
            ...
            if cmd.startswith('shell'):
                cmd = ["adb", "-s", Var.desired_caps.udid, "shell",
                       "{}".format(cmd.lstrip('shell').strip())]
    ...
    def launch_app(package_info):
        ...
        try:
            if not package_info:
                AndroidDriver.adb_shell('shell am start -W {}/{}'.format(Var.desired_caps.package,
                                                                         Var.desired_caps.activity))
            else:
    ...
    def swipe(from_x, from_y, to_x, to_y, duration=2):
        ...
        try:
            ...
            AndroidDriver.adb_shell(
                'shell input swipe {} {} {} {} {}'.format(from_x, from_y, to_x, to_y,
                                                          duration * 100))
        except Exception as e:
            raise e
    ...
    def wait_for_elements_by_id(id, timeout=10, interval=1):
        ...
        """
        try:
            elements = Var.instance.wait_for_elements_by_id(id, int(timeout) * 1000,
                                                            int(interval) * 1000)
            return elements
        except:
            return None
    ...
    def wait_for_elements_by_name(name, timeout=10, interval=1):
        ...
        """
        try:
            elements = Var.instance.wait_for_elements_by_name(name, int(timeout) * 1000,
                                                              int(interval) * 1000)
            return elements
        except:
            return None
    ...
    def wait_for_elements_by_xpath(xpath, timeout=10, interval=1):
        ...
        """
        try:
            elements = Var.instance.wait_for_elements_by_xpath(xpath, int(timeout) * 1000,
                                                               int(interval) * 1000)
            return elements
        except:
            return None
    ...
    def wait_for_elements_by_classname(classname, timeout=10, interval=1):
        ...
        """
        try:
            elements = Var.instance.wait_for_elements_by_class_name(classname, int(timeout) * 1000,
                                                                    int(interval) * 1000)
            return elements
        except:
            return None
...
class iOSDriver(object):
    ...
    def swipe(from_x, from_y, to_x, to_y, duration=2):
        ...
        try:
            ...
            if to_y <= 1.0:
                to_y = to_y * height
            Var.instance.touch('drag', {'fromX': from_x, 'fromY': from_y, 'toX': to_x, 'toY': to_y,
                                        'duration': duration})
        except Exception as e:
            raise e
