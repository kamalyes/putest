...
from putest.keywords import keywords
from putest.result.test_runner import TestRunner
from putest.runner.run_case import RunCase
from putest.utils import *
...
class Project(object):
    ...
    def _init_config(self):
        ...
        if Var.driver != 'selenium':
            ...
            if not Var.desired_caps.browser or Var.desired_caps.browser not in ['chrome', 'safari',
                                                                                'firefox', 'ie',
                                                                                'opera',
                                                                                'phantomjs']:
            ...
            if self._workers > 1:
                if isinstance(Var.desired_caps.udid, list):
                    if not Var.desired_caps.udid:
                        raise Exception(
                            'Can‘t find device, udid("{}") is empty.'.format(Var.desired_caps.udid))
                    ...
                    if not Var.desired_caps.udid[self._index]:
                        raise Exception('Can‘t find device, udid("{}") is empty.'.format(
                            Var.desired_caps.udid[self._index]))
                    devices = DevicesUtils(Var.desired_caps.platformName,
                                           Var.desired_caps.udid[self._index])
    ...
    def _analytical_common_file(self):
        ...
        for rt, dirs, files in os.walk(common_dir):
            ...
            elif Var.desired_caps.platformName and (
                    rt.split(os.sep)[-1].lower() == Var.desired_caps.platformName.lower()):
                self._load_common_func(rt, files)
