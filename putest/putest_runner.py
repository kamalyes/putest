...
from putest.project import *
from putest.version import VERSION


def _usage():
    print('')
    print('  usage: putest [-h|-v|] [arg] ...')
    print('')
    print('  options:')
    print('')
...
def _init_project(dir):
    try:
        ...
        dirs = ['Common/Android', 'Common/iOS', 'Common/Selenium', 'Resource', 'Scripts',
                'TestCase']
        for dir_ in dirs:
...
def main():
    ...
    """
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'hVi:r:w:',
                                   ['help', 'Version', 'init=', 'run=', 'workers='])
    except:
        _usage()
    project_path = '.'
    workers = 1
