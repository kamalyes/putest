...
class TestCase(unittest.TestCase):
    ...
    def __init__(self, methodName="runTest"):
        ...
        self.snapshot_dir = os.path.join(Var.report, 'Steps', self.module,
                                         self.test_case_path.split('/')[-1].split(os.sep)[-1].split(
                                             ".")[0])
        ...
        try:
            ...
            with open(self.test_case_path, 'r', encoding='UTF-8') as r:
                ...
                for step in s[index + 1:]:
                    if not (step.lstrip().startswith('#') or re.match('#', step.lstrip().lstrip(
                            '-').lstrip())):
                        if step != '\n':
                            testcase_steps.append(step)
            log_info("******************* TestCase {} Start *******************".format(
                self.description))
            unittest.TestCase.run(self, result)
            log_info(
                "******************* Total: {}, Success: {}, Failed: {}, Error: {}, Skipped: {} ********************\n"
                .format(result.testsRun, len(result.successes), len(result.failures),
