...
from putest.runner.action_executor_base import ActionExecutorBase
...
class ActionExecutorApp(ActionExecutorBase):
    ...
    def _swipe(self, action):
        ...
        try:
            ...
            raise TypeError(
                'swipe takes 1 positional argument but {} were giver'.format(len(action.step)))
    ...
    def _getElement(self, action):
        ...
        if isinstance(parms, WebElement):
            ...
            element = DriverBaseApp.find_elements_by_key(key=parms, timeout=Var.time_out,
    ...
    def _getElements(self, action):
        ...
        """
        parms = self._getParms(action, 0)
        elements = DriverBaseApp.find_elements_by_key(key=parms, timeout=Var.time_out,
                                                      interval=Var.interval,
                                                      not_processing=True)
        if not elements:
            raise Exception("Can't find element {}".format(parms))
    ...
    def _isNotExist(self, action):
        ...
        if not isinstance(img_info, bool):
            ...
            elements = DriverBaseApp.find_elements_by_key(key=parms, timeout=0,
                                                          interval=Var.interval,
                                                          not_processing=True)
            result = bool(elements)
        return not result
